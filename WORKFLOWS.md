...
| `.github/workflows/ent-dependabot-weekly.yml` | Weekly autonomous closeout for Dependabot PRs across the dynamic Merglbot ENT scope. | Scheduled Monday 07:30 UTC + manual `workflow_dispatch`. Calls `.github/workflows/ent-dependabot-autonomous-closeout.yml`. | Scheduled runs use `apply`; manual runs default to `dry-run`. In GitHub Actions, scope is resolved from `merglbot-public/docs/ENT_ORG_ALLOWLIST.md` plus live non-archived/non-fork repository metadata. | Uses the ENT Dependabot GitHub App secrets when present, with `secrets.ENTERPRISE_GITHUB_TOKEN` as a legacy fallback. Branch/ruleset alignment requires a token with repo administration capability. |
