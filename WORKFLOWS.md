...
| `.github/workflows/reusable-deploy-cloud-run.yml` | Build → push → deploy container to Cloud Run via WIF. | `workflow_call` from service repos once CI checks pass. | Inputs: `service`, `region`, `project_id`, `service_account`, `environment`, `dockerfile`, `env_vars`, `secrets`. Emits artifact digest via job summary. | Secrets: `GCP_WIF_PROVIDER`, `GCP_WIF_SERVICE_ACCOUNT`, `GAR_LOCATION`. |
...
| `.github/workflows/markdown-danger-lint.yml` | Lints Markdown PRs for dangerous `git push --all --force` guidance and formatting issues. | `pull_request` (opened/edited/synchronize). | No inputs; auto-detects changed `.md` files. | No secrets. Status name `lint`. |
