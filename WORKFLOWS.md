...
| `.github/workflows/reusable-policy-validation.yml` | Policy-as-code validation for workflow security (Conftest/OPA). | `workflow_call` from consumer repos as a required PR check. | Input `workflows_glob` (defaults to validating `.github/workflows/*.yml/.yaml`). Policies are loaded from this repo pinned to the called workflow commit SHA (no `@main` drift). | No secrets. Recommended job name `policy`. |
...
| `.github/workflows/forecast-d1-readiness.yml` | Forecast D‑1 readiness check for final tables (inventory-driven) with Slack PASS/FAIL notifications. | Scheduled 3× daily (08:00/10:00/15:00 Europe/Prague, DST-stable via dual UTC cron) + manual dispatch (`patch_date_local`, `dry_run`). | 08/10 required scope = `country=cz` only (non-CZ informational); 15 required scope = all. Produces artifacts: CSV/MD/JSON report. | Uses WIF (`vars.GCP_WIF_PROVIDER`, `vars.GCP_WIF_SERVICE_ACCOUNT`). Slack via `secrets.SLACK_WEBHOOK_URL` (optional; skips if missing). |
...
### Required Status Checks (`[build, test, codeql, policy]`)

- Consumer repositories must expose four jobs named `build`, `test`, `codeql`, `policy`.  
- `codeql` jobs should call `reusable-codeql-analysis.yml`.  
- `policy` jobs should call `reusable-policy-validation.yml`.  
