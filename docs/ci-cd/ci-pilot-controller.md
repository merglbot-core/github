...
Standard-library Python; no scheduler install, reviews, merges or bot repairs.
Stop the old monitor before installing a periodic runner. Share one durable
state directory; never reset its distinct-PR case history.
...
cancellations. Samples may miss brief waits; these are not causal savings.
Schedule frequently enough to observe ten minutes. No model is needed per tick;
new semantic receipts need an agent. Persist no diff, transcripts or raw logs.
