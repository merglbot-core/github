...
process_repo() {
  ...
  local open_pr
  if ! open_pr=$(gh pr list -R "$repo" --head ci/docs-governance-advisory --state open \
      --json number,headRepositoryOwner --jq "[.[]|select(.headRepositoryOwner.login==\"${repo%%/*}\")]|length" 2>/dev/null); then
